...
const weatherByCity = async (geoAPIResponse) => {
    ...
    const url = `https://api.openweathermap.org/data/2.5/onecall?lat=${cityLat}&lon=${cityLon}&exclude=minutely,hourly&units=imperial&appid=8e70806bdc0d747fc69d3ace5e3331a1`
...
const geoCode = async (cityName) => {
    const url = 'https://api.openweathermap.org/data/2.5/forecast?id=524901&appid?q=' + cityName + '&appid=8e70806bdc0d747fc69d3ace5e3331a1'
...
const currentWeatherDOM = async (weatherAry) => {
    const currEl = document.getElementById('main-box')
...
document.getElementById('search-area').addEventListener('submit', searchForm)